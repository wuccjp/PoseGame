...
def update_states(detected: set, now: float):
    """依據本幀偵測結果更新每個角落的狀態機。
    觸發規則：達 2 秒的角落 → 只有該角落變 TRIGGERED（紅底倒數）。
    鎖定規則：任一角落觸發後，全域鎖定啟動，全部角落停止感應；
              待該角落 5 秒倒數結束後，全域鎖定解除，全部恢復 IDLE。
    """
    global global_lockout_ts

    # ── 全域鎖定期間 ──
    if global_lockout_ts > 0.0:
        # 找出正在 TRIGGERED 的角落，看倒數是否結束
        still_triggered = any(
            st["state"] == TRIGGERED and now - st["ts"] < LOCKOUT_SEC
            for st in corner_states.values()
        )
        if not still_triggered:
            # 所有 TRIGGERED 都結束 → 解除全域鎖定，全部回 IDLE
